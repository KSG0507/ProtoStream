...
%--------------------------------------------------------------------------
    ...
    methods (Access = public)
        ...
        function InsertKey(this,key)
            ...
            %               
            % Inputs:       key is a structure with (key,value) pair
            %               
        ...
        %
        function ReplaceMin(this,key)
            %------------------------ ReplaceMin --------------------------
            % Syntax:       min = H.ReplaceMin(key);
            %               
            % Inputs:       key is a structure with (key,value) pair 
            %
            % Description:  Replace the minimum key in H 
            %--------------------------------------------------------------
            this.x{1} = key;
            this.MinHeapify(1);
        end
    ...
    methods (Access = private)
        ...
        function DecreaseKey(this,i,key)
            ...
            elseif (key.value > this.x{i}.value)
                % Decrease key error
                MinHeap.DecreaseKeyError();
            end
            this.x{i} = key;
            while ((i > 1) && (this.x{Heap.parent(i)}.value > this.x{i}.value))
        ...
        function MinHeapify(this,i,size)
            ...
            ll = Heap.left(i);
            rr = Heap.right(i);
            if ((ll <= size) && (this.x{ll}.value < this.x{i}.value))
                smallest = ll;
            else
                smallest = i;
            end
            if ((rr <= size) && (this.x{rr}.value < this.x{smallest}.value))
                smallest = rr;
            end
            if (smallest ~= i)

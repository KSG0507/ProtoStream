...
for labelCount = 1:numDiffLablesToRun
    ...
    for prop = 1:length(proportions)
        ...
        for kcount = 1:numkValues
            k = kValues(kcount);
            if(k==m)
                [~,S_HX,setValue_HX,~] = ProtoDashStreaming(X,Y,m,kernelType,sigmaVal,meanInnerProductX);
            else
                [~,S_HX,setValues_HX,~] = HeuristicSetSelection(X,Y,m,kernelType,sigmaVal,meanInnerProductX,k);
            end
        ...
        if(runStreaming)
            ...
            classifiedLabels_HS = NNC(X,Y(:,S_HS(1:setSize_PDS)),labels_Y(S_HS(1:setSize_PDS)),startSparsity);
        end

function [chosenWeights,chosenSet,chosenSetValue,MultipleWeights,MultipleSets,MultipleSetValues,setPosWithEnoughElements,stageWeights,stageGradients] = ProtoDashStreamingWithThreshold_Variation1(X,Y,m,kernelType,th,varargin) 
    ...
    for count = 1:numRemaining
        ...
        % gradient values
        for j = min(sizeS,m-1):-1:setPosWithEnoughElements
            if(j==0)
            ...
            if(H{j+1}.Count == m-j-1)
                H{j+1}.InsertKey(element);
                setPosWithEnoughElements = j;
                break;
            else
                if(H{j+1}.Count() < m-j)
                    H{j+1}.InsertKey(element);
        ...
        if((gradientVal >=sqrt(2*th/m)) && sizeS < m)
            ...
            if(sizeS < m)
                ...
                setPosWithEnoughElements = m;
                fprintf('Reached a set with %d elements. Hence coming out of the stream\n',m);
                break;
            end
        end
    end
    ...
    fprintf('Position on the set with enough elements  = %d\n',setPosWithEnoughElements);
    timeForOptimalset = tic;
    for j = sizeS:-1:setPosWithEnoughElements

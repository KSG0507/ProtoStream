...
%%
load(strcat('Variables_',outputFileName,'_Run1'));
%%
...
if(runStreaming)
    spLevelForStreaming = min(setSize_PDS)-startSparsity+1;
	plot(startSparsity:min(setSize_PDS),a_HSMean(1:spLevelForStreaming),'m-','Linewidth',2);
end
...
hold off;
titleString = sprintf('UCI performance');
title(titleString,'fontsize',axesFontSize,'fontweight','bold');
xlabel('Sparsity level','fontsize',axesFontSize,'fontweight','bold');
ylabel('Classification Accuracy','fontsize',axesFontSize,'fontweight','bold');
if(plotBudget && runBudget)
	if(runGreedy)
		leghan = legend('ProtoDash','L2C','L2C Adpated','Random','K-Medoids','P-Lasso','ProtoGreedy');
	else
        ...
        leghan = legend('ProtoDash','ProtoBasic','ProtoStream','L2C','L2C-A','RndW','K-Med', 'P-Las');
	end
else
	if(runGreedy)
		leghan = legend('ProtoDash','L2C','L2C Adpated','Random','K-Medoids','ProtoGreedy');
	else
        ...
        leghan = legend('ProtoDash','ProtoBasic','ProtoStream','L2C','L2C-A','RndW','K-Med');
	end
end
...
hold off;
titleString = sprintf('a) UCI performance');
title(titleString,'fontsize',axesFontSize,'fontweight','bold');

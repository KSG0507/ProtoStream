function [currOptw_U,setValues_U,currOptw_E,setValues_E,S] = RandomStreamingSetSelection(X,Y,m,kernelType,varargin) 
    ...
    currK = [];
    curru = [];
    %% Perform reservoir sampling to select m prototypes
    i = 1;
    while(i <= numY)
        if(i <= m)
            S(i) = i;
            ...
            if(rand <= m/i)
                positionToReplace = randi(m);
                S(positionToReplace) = i;
            end
        end
        i = i+1;
    end     

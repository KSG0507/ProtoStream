...
if(runCode)
    ...
    if(saveOutput)
        ...
        if(runGreedyStreaming)
            save(strcat('Variables_',outputFileName),'a_PD','a_PDS','protoLabels_PD','protoLabels_PDS','setValue_PD','setValue_PDS','setSize_PDS','a_PGS','protoLabels_PGS','setValue_PGS','setSize_PGS','S_PD','S_PDS','S_PGS');
        else
            save(strcat('Variables_',outputFileName),'a_PD','a_PDS','protoLabels_PD','protoLabels_PDS','setValue_PD','setValue_PDS','setSize_PDS','S_PD','S_PDS');
        end
    end
else
...
if(plotFigure)
    ...
    %plot(sparsityValues(startLoc:end),a_PDMean(startLoc:end,1),'b-','Linewidth',2);
    %hold on;
    ...
    axis tight;
    if(runGreedyStreaming)
        legend('ProtoBasic','ProtoStream','Streak');
    else
